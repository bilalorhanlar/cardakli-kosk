...
export async function uploadImage(file, itemName) {
  try {
    ...
    const command = new PutObjectCommand({
      ...
      ACL: 'public-read' // Resimlere herkesten erişim izni ver
    });
...
export async function getImageUrl(fileName) {
  try {
    ...
    // 24 saat geçerli olacak şekilde URL oluştur
    const url = await getSignedUrl(s3Client, command, { 
      expiresIn: 86400, // 24 hours
      // S3 URL'sini daha temiz hale getir
